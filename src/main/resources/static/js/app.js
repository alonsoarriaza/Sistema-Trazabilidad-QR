 ...
 */
var maquinaActualEnFicha = null;
var piezasActualesEnFicha = [];

function cargarComponentesEHistorialMaquina(maquina) {
    maquinaActualEnFicha = maquina;
    ...
    const headerColor = document.getElementById('extHeaderEsColorCheckbox');
    ...
    if (headerColor) headerColor.checked = maquina.esColor === true;
...
function toggleEsColorMaquina(checked) {
    if (!maquinaActualEnFicha) return;
    maquinaActualEnFicha.esColor = checked;

    fetch(API_BASE + '/maquinas/' + maquinaActualEnFicha.id + '?tecnico=' + encodeURIComponent(obtenerTecnico()), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(maquinaActualEnFicha)
    })
    .then(function(res) {
        if (res.ok) return res.json();
        throw new Error('Error al actualizar propiedad de color');
    })
    .then(function(m) {
        maquinaActualEnFicha = m;
        renderizarListaComponentes(m, piezasActualesEnFicha, m.esColor === true);
        mostrarToast('Configuración de color actualizada: ' + (m.esColor ? 'Color (4 Tambores / 4 Reveladores)' : 'Monocromo (1 Tambor K / 1 Revelador K)'), 'info');
    })
    .catch(function(err) {
        console.error(err);
        mostrarToast('No se pudo guardar el cambio de color.', 'danger');
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-activar o desactivar el checkbox de Color según se ingresen o borren copias en color
    const inputCopiasColor = document.getElementById('maqCopiasColor');
    if (inputCopiasColor) {
        inputCopiasColor.addEventListener('input', function() {
            const val = parseInt(this.value, 10);
            const checkColor = document.getElementById('maqEsColor');
            if (checkColor) {
                if (!isNaN(val) && val > 0) {
                    if (!checkColor.checked) {
                        checkColor.checked = true;
                        guardarBorradorMaquina();
                    }
                } else {
                    if (checkColor.checked) {
