/* ═══════════════════════════════════════════════════════════════════
   LÓGICA PRINCIPAL — Sistema de Trazabilidad QR trazabilidad
...
function renderTablasMaquinas(maquinas) {
    ...
    var html = '<table class="table table-dark-trazabilidad table-hover">';
    html += '<thead><tr>';
...
/**
 * Carga las piezas extraídas de una máquina concreta (trazabilidad inversa).
 *
...
function buscarGlobal(isAuto) {
    ...
    Promise.all([fetchInventario, fetchPiezas, fetchToners]).then(function(resultados) {
        ...
        if (piezas.length > 0) {
            ...
            var htmlPza = '<table class="table table-dark-trazabilidad table-hover">';
        ...
        if (toners.length > 0) {
            ...
            var htmlToner = '<table class="table table-dark-trazabilidad table-hover">';
...
function cerrarSesion(event) {
    ...
    localStorage.removeItem('trazabilidad_session');
...
function obtenerTecnico() {
    return localStorage.getItem('trazabilidad_tecnico') || 'Técnico';
}
...
function guardarTecnico(nombre) {
    if (nombre) {
        localStorage.setItem('trazabilidad_tecnico', nombre);
...
function mostrarModalTecnico() {
    ...
    if (modalEl) {
        document.getElementById('inputNombreTecnico').value = localStorage.getItem('trazabilidad_tecnico') || '';
...
function guardarBorradorMaquina() {
    ...
    localStorage.setItem('trazabilidad_draft_maquina', JSON.stringify(borrador));
}
...
function restaurarBorradorMaquina() {
    ...
    const borradorStr = localStorage.getItem('trazabilidad_draft_maquina');
    if (borradorStr) {
        try {
...
function limpiarBorradorMaquina() {
    localStorage.removeItem('trazabilidad_draft_maquina');
}
...
function guardarBorradorPieza() {
    ...
    localStorage.setItem('trazabilidad_draft_pieza', JSON.stringify(borrador));
}
...
function restaurarBorradorPieza() {
    ...
    const borradorStr = localStorage.getItem('trazabilidad_draft_pieza');
    if (borradorStr) {
        try {
            const borrador = JSON.parse(borradorStr);
            Object.keys(borrador).forEach(function(idCampo) {
...
function limpiarBorradorPieza() {
    localStorage.removeItem('trazabilidad_draft_pieza');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tecnicoActual = localStorage.getItem('trazabilidad_tecnico');
    if (!tecnicoActual) {
        setTimeout(function() {
            mostrarModalTecnico();
        }, 300);
    ...
    window.addEventListener('beforeunload', function (e) {
        ...
        if (!maqId) {
            const borradorMaq = localStorage.getItem('trazabilidad_draft_maquina');
            if (borradorMaq) {
                try {
        ...
        if (!pzaId) {
            const borradorPza = localStorage.getItem('trazabilidad_draft_pieza');
            if (borradorPza) {
                try {
...
function renderTablaToners(toners) {
    ...
    let html = '<table class="table table-dark-trazabilidad table-hover">';
    html += '<thead><tr>';
