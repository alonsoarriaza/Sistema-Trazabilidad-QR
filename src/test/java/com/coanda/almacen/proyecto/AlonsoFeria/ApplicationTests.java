package com.coanda.almacen.proyecto.AlonsoFeria;
...
class ApplicationTests {
	...
	void printTableColumns() {
		try {
			java.util.List<Object> maquinasCols = entityManager.createNativeQuery(
					"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'almacen_coanda' AND TABLE_NAME = 'maquinas'")
					.getResultList();
			System.out.println("COLUMNAS DE MAQUINAS: " + maquinasCols);

			java.util.List<Object> piezasCols = entityManager.createNativeQuery(
					"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'almacen_coanda' AND TABLE_NAME = 'piezas'")
					.getResultList();
			System.out.println("COLUMNAS DE PIEZAS: " + piezasCols);

			java.util.List<Object> tonersCols = entityManager.createNativeQuery(
					"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'almacen_coanda' AND TABLE_NAME = 'toners'")
					.getResultList();
	...
	void testEscrituraPieza() {
		...
		maquinaOrigen.setClienteProcedencia("Cliente coanda");
	...
	@Autowired
	private com.coanda.almacen.proyecto.AlonsoFeria.servicio.MaquinaService maquinaService;
	...
	@Autowired
	private com.coanda.almacen.proyecto.AlonsoFeria.repositorio.TonerRepository tonerRepository;

	@Autowired
	private com.coanda.almacen.proyecto.AlonsoFeria.servicio.TonerService tonerService;
	...
	void addReservationColumnsToMaquinas() throws Exception {
		try (java.sql.Connection conn = dataSource.getConnection();
			...
			java.sql.ResultSet rs = stmt.executeQuery(
				...
				"WHERE TABLE_SCHEMA='almacen_coanda' AND TABLE_NAME='maquinas' " +
	...
	void testBuscarToner() {
		com.coanda.almacen.proyecto.AlonsoFeria.modelo.Toner t1 = new com.coanda.almacen.proyecto.AlonsoFeria.modelo.Toner();
		...
		java.util.List<com.coanda.almacen.proyecto.AlonsoFeria.modelo.Toner> resModelo = tonerService.buscar("tn24");
		assertFalse(resModelo.isEmpty());
		assertEquals("TNR-TEST-123", resModelo.get(0).getCodigoQr());

		// Buscar por QR (case-insensitive)
		java.util.List<com.coanda.almacen.proyecto.AlonsoFeria.modelo.Toner> resQr = tonerService.buscar("test-123");
